...
val9 = st.selectbox("Childhood Autism Rating Scale  ",d1)
val9 = ValueCount(val9)

val10 = st.selectbox("Anxiety disorder  ",d3)
val10 = ValueCount(val10)

d4=["Female","Male"]
val11 = st.selectbox("Gender  ",d4)
val11 = Sex(val11)

val12 = st.selectbox("Suffers from Jaundice ",d3)
val12 = ValueCount(val12)

val13 = st.selectbox("Family member history with ASD  ",d3)
...
with st.expander("Analyze provided data"):
  ...
  if (prediction[0] == 0):
    st.info('The is not with Autism spectrum disorder' )
  else:
    st.warning('The person is with Autism spectrum disorder' )